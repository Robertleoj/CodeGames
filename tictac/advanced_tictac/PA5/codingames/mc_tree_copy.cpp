...
void AgentState::move(Move *m){
    ...
    this->move_sequence.push_back(*m);
    switch_turns();
...
void AgentState::undo_move(){
    ...
    Move last_move = move_sequence.back();
...
std::vector<Move> AgentState::get_available_moves(){
            ...
            for(int i = 0; i < 3; i ++ ){
                for(int j = 0; j < 3; j++){
                    if(board[last_move[2]][last_move[3]][i][j] == EMPTY){
                        available_moves.push_back(Move(last_move[2], last_move[3], i, j));
                    }
                }
            }
...
void Agent::update(board_idx move){
    // Update state
    Move m = this->board_idx_to_move(move);
    this->update(m);
}
...
Condition Agent::simulation(MCNode *selected_node) {
    ...
    while (true) {
        ...
        // Select random move
        avail_moves = state->get_available_moves();
        rand_idx = rand() % avail_moves.size();
        state->move(&avail_moves[rand_idx]);
        cnt ++;
    }
...
Move move_from_2d_coord(int x, int y){
    int x1 = x / 3;
    int x2 = x % 3;
    int y1 = y / 3;
    int y2 = y % 3;
...
int main(int argc, char *argv[])
{
    ...
    while (2) {
        ...
        if(opponent_row != -1){
            Move m = move_from_2d_coord(opponent_row, opponent_col);
            agent.update(m);
        }
